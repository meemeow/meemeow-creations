...
export default function ProjectCard({ p, isEven }: { p: Project; isEven: boolean }) {
  ...
  return (
    <div className={`flex flex-col md:flex-row items-center gap-6 w-full ${isEven ? "md:flex-row-reverse" : ""}`}>
      <div
        className={`${CARD_SHELL} ${cardBackground(hasPreview, true)} aspect-[16/8] h-auto w-full md:w-full [transition:transform_200ms_ease,box-shadow_200ms_ease] [.project-item:hover_&]:[transform:translateY(-4px)] [.project-item:hover_&]:[box-shadow:0_18px_36px_rgba(0,0,0,0.6)] above-467:upto-940:aspect-[4/2] above-467:upto-940:min-h-48 upto-467:aspect-[4/3] upto-467:min-h-32 upto-467:max-w-full`}
